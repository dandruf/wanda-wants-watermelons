--[[
    Represents our player in the game, with its own sprite. NOT IMPLEMENTED YET!
]]
...
function Player:update(dt)
    ...
    if self.y > (self.map.mapHeight * self.map.tileHeight) then
        ...
        ministate = 'die'
    elseif self.x < (self.map.mapWidthPixels - (self.map.tileWidth)) and self.x > (self.map.mapWidthPixels - (self.map.tileWidth * 2)) then
        ministate = 'win'
